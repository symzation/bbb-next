...
export default function AuthorForm({ 
  ...
  return (
    <>
      <h1 className="text-3xl font-bold mb-4">Apply To Become A Reviewer?</h1>
      <form 
        action={formAction} 
        className="flex flex-col justify-center items-center gap-4 w-full"
      >
        <div className="flex flex-col items-center w-full min-h-52">
          <div 
            ref={authorTermsRef}
            id="termsContainer"
        ...
        <div className="relative flex flex-col justify-start items-center w-full">
          <div className="flex justify-start items-center w-full pl-1">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button 
                  variant="ghost" 
                  className="text-left p-0 inline-block font-bold"
                >
          ...
          <input type="text" name="penName" defaultValue=""
            placeholder="Pen Name" className={cn(styles.formInput, 'pl-[35px]')}
          />
          {formState?.errors && typeof formState.errors === "object" && !Array.isArray(formState.errors) && "penName" in formState.errors && (
            <div className="text-error text-sm italic mt-1">
              {formState.errors.penName}
            </div>
          )}
        </div>
        <TextareaInput 
          inputClassName="mt-8 w-full h-40"  
        ...
        <div className="flex flex-col gap-1 w-full">
          <div className="flex items-center gap-3">
            <input 
              ...
              id="reviewerTerms"
              name="reviewerTerms" 
              className="data-[state=checked]:bg-primary data-[state=checked]:text-white w-6 h-6" 
              defaultChecked={termsCheckedValue}
              onChange={handleReviewerTermsChange}
            />
            <Label htmlFor="reviewerTerms" className="font-bold tracking-wideest">
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-4 mt-4">
          <Button 
            type="submit" 
            disabled={isPending}
            className={cn("w-full text-white tracking-wider w-2/5 py-5 px-12 ring-0 focus:ring-0 ring-offset-0 focus:ring-offset-0 focus-visible:ring-0 outline-none cursor-pointer data-[state=open]:bg-transparent", isPending && "opacity-70 cursor-not-allowed")}
          >
            {isPending ? "Submitting.." : "Become A Reviewer"}
          </Button>
          <Button 
            className="w-full text-white tracking-wider w-2/5 py-5 px-12 ring-0 focus:ring-0 ring-offset-0 focus:ring-offset-0 focus-visible:ring-0 outline-none cursor-pointer"
